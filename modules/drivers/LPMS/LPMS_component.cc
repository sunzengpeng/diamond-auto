...
bool LPMSDriverComponent::m_sensorThread_fun(
    const SensorThreadParams& param)  // read data from IMU
{
...
bool LPMSDriverComponent::Init(/*std::shared_ptr<apollo::cyber::Node> node*/) {
  ...
  if (m_sensorName.size() == 0) {
    ...
    // listing
    if (m_baudrate > 0) foundSens.baudRate = m_baudrate;
...
void LPMSDriverComponent::publishIsAutocalibrationActive() {
  ...
  if (!m_zenImu) {
    AINFO
        << "No IMU compontent available, can't publish autocalibration status";
    return;
  }
...
bool LPMSDriverComponent::setAutocalibration(
    const std::shared_ptr<Request>& req, const std::shared_ptr<Content>& res) {
  ...
  if (auto error = m_zenImu->setBoolProperty(
    ...
    msg.append(std::string("[Failed] current autocalibration status set to: ") +
               (req->data() ? "True" : "False"));
...
bool LPMSDriverComponent::resetHeading(const std::shared_ptr<Content>& request,
  ...
  // 2: Alignment reset
  if (auto error =
          m_zenImu->setInt32Property(ZenImuProperty_OrientationOffsetMode, 1)) {
    AINFO << "Error: " << error;
...
bool LPMSDriverComponent::calibrateGyroscope(
    const std::shared_ptr<Content>& req, const std::shared_ptr<Content>& res) {
  if (!m_zenImu) {
    AINFO << "No IMU compontent available, can't start autocalibration";
    return false;
  }

  AINFO << "calibrate_gyroscope: Please make sure the sensor is stationary for "
           "4 seconds";
